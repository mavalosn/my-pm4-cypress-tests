...
export class Execution {
    ...
    actionsAndAssertionsOfTCP42105(requestId){
        ...
        cy.xpath('//button[text()="Claim Task"]').click();
        cy.wait(5000);
        cy.xpath('//input[@name="var1"]').clear().type('1');
        cy.xpath('//button[@aria-label="New Submit"]').click();
        navHelper.navigateToLogOut();
        login.login();
        cy.visit('/requests/'+requestId);
        ...
        cy.xpath('//button[@aria-label="New Submit"]').click();
        cy.xpath('//button[@aria-label="New Submit"]').click();
        request.waitUntilElementIsVisibleCant('selector', "[class='py-3 d-flex']",9);
        request.waitUntilTextcontainText('selector','h4','Completed');
    }
    ...
    actionsAndAssertionsOfTCP42158(requestId){
        ...
        if(day >= 10){
            ...
            cy.xpath('//div[contains(text(),"Admin User has completed the task B")]').should('be.visible');
        }else{
            cy.xpath('(//input[@placeholder="Search"])[1]').type('TCP4-2158 Verify Event Based Gateway Conditional and Message Event').type('{enter}');
            cy.xpath('(//span[text()="TCP4-2158 Verify Event Based Gateway Conditional and Message Event"]/ancestor::tr/td)[1]').should('be.visible').invoke('text').then((text) => {
                var requestName = text.trim();
                requestName = requestName.substring(
                    1,
                    requestName.length);
                    cy.visit('/requests/'+requestName);
                    request.clickOnTaskName(1, 1);
                    cy.xpath('//button[@aria-label="New Submit"]').click();
                    cy.visit('/requests/'+requestId);
                    request.clickOnTaskName(1, 1);
                    cy.xpath('(//button[@aria-label="New Submit"])[2]').click({force:true});
                    cy.visit('/requests/'+requestId);
                    request.waitUntilElementIsVisibleCant('selector', "[class='py-3 d-flex']",4);
                    cy.xpath('//div[contains(text(),"Admin User has completed the task C")]').should('be.visible');
                })
        }
    }
    ...
    actionsAndAssertionsOfTCP42217(requestId){
        ...
        cy.xpath('//input[@aria-label="New Date Picker"]').type('02022022', {force:true}).type(' ').type('{enter}');
        cy.xpath('//input[@data-cy="screen-field-input"]').type('Roma');
        cy.xpath('//div[@class="multiselect__select"]').click();
        cy.xpath('//li[@aria-label="aa. "]/span/span[text()="aa"]').click();
        ...
        if(day >= 10){
            cy.visit('/requests/'+requestId);
            request.clickOnTaskName(1, 1);
            cy.xpath('//input[@data-cy="screen-field-input"]').should('have.value', 'Roma');
...
//////////////////////
    ...
    actionsAndAssertionsOfTCP42255(requestId) {
        //Complete form A
        //fill input
        cy.get('input[aria-label="New Input"]').should('be.visible').click();
        ...
        cy.get('[aria-label="New Textarea"]').type('Input in text area');
        //click on submit button
        cy.xpath('//button[@class="btn btn-primary"]').click();
        //verify task is completed
        cy.get('[class="alert d-none d-lg-block alertBox alert-dismissible alert-success"]').should('be.visible');
        ...
        //open request by ID
        cy.visit('/requests/' + requestId);
        request.waitUntilElementIsVisible('selector','#pending > div > div > table > tbody > tr > td:nth-child(2) > a',32);
        request.clickOnTaskName(1, 1);
        //click on submit button
        cy.xpath('//button[@class="btn btn-primary"]').click();
        //verify task is completed
        ...
        //verify the process is completed
        request.verifyRequestisCompleted(requestId);
        cy.contains('Admin User has completed the task B').scrollIntoView()
        cy.xpath("//div[contains(text(),'Admin User has completed the task A')]").should('be.visible');
        cy.xpath("//div[contains(text(),'Admin User has completed the task PDF Generator A')]").should('be.visible');
        ...
        cy.xpath("//div[contains(text(),'Admin User has completed the task PDF Generator C')]").should('be.visible');
        cy.xpath("//div[contains(text(),'Admin User has completed the task Send Email C')]").should('be.visible');
        cy.xpath("//div[contains(text(),'Admin User has completed the task PDF Generator B')]").should('be.visible');
        cy.xpath("//div[contains(text(),'Admin User has completed the task Send Email D')]").should('be.visible');
        cy.xpath("//div[contains(text(),'Admin User has completed the task PDF Generator D')]").should('be.visible');
        cy.xpath("//div[contains(text(),'Admin User has completed the task B')]").should('be.visible');
    }
    ...
    async CompleteFormTCP42240(requestId){
        ...
        request.clickOnTaskName(1, 1);
        cy.wait(3000);
        cy.xpath('//div[@class="custom-css-scope"]').first().should("be.visible");
        //Add new loop
        cy.get('[data-cy="loop-loop_1-add"]').should("be.visible").click();
    ...
    async CompleteFormTCP42268(){
       cy.get('[class="card card-body border-top-0 h-100 form-screen"]').should("be.visible");
       cy.xpath('//text()[contains(.,"New Date Picker")]/ancestor::div[@data-cy="screen-field-form_date_picker_1"]//input').should("be.visible").type('2022-09-30 19:01{enter}');
       cy.xpath("//strong[text()='Nested Validations']").should("be.visible");
       //Options Menu
       cy.xpath("//input[@type='text'][@name='form_input_1']").should("be.visible");
       cy.xpath("//input[@type='text'][@name='form_input_2']").should("be.visible").type('place holder').should('have.value','place holder');
       cy.xpath("//input[@type='text'][@name='form_input_3']").should("be.visible").type('helper text').should('have.value','helper text');
       cy.xpath("//small[text()='helper text']").should("be.visible");
       cy.xpath("//input[@type='text'][@name='design']").should("be.visible").type('design').should('have.value','design');
       cy.xpath("//input[@type='text'][@name='form_input_5']").should("be.visible").should('have.value','paos default  value');
       cy.xpath("//input[@type='text'][@name='form_input_4']").should("be.visible").type('30/09/1993{enter}').should('have.value','30/09/1993');
       cy.xpath("//input[@type='text'][@name='form_input_7']").should("be.visible").type('300-05-1993{enter}').should('have.value','300-05-1993');
       //Validations Rules
       cy.xpath("//span[text()='Validations Rules']").should("be.visible");
       cy.xpath('//text()[contains(.,"dateStart")]/ancestor::div[@data-cy="screen-field-dateStart"]//input').should("be.visible").type('2022-09-30{enter}').should('have.value','2022-09-30');
       cy.xpath("//input[@type='text'][@name='form_input_8']").should("be.visible").type('yes').should('have.value','yes');
       cy.xpath("//input[@type='text'][@name='form_input_9']").should("be.visible").type('2022-10-30{enter}').should('have.value','2022-10-30');
       cy.xpath("//input[@type='text'][@name='form_input_10']").should("be.visible").type('2022-09-30{enter}').should('have.value','2022-09-30');
       cy.xpath("//input[@type='text'][@name='Alpha']").should("be.visible").type('Mayita').should('have.value','Mayita');
       cy.xpath("//input[@type='text'][@name='AlphaNumeric']").should("be.visible").type('Mayita123').should('have.value','Mayita123');
       cy.xpath("//input[@type='text'][@name='BeforeDate']").should("be.visible").type('2022-08-30{enter}').should('have.value','2022-08-30');
       cy.xpath("//input[@type='text'][@name='BeforeorEqualtoDate']").should("be.visible").type('2022-08-30{enter}').should('have.value','2022-08-30');
       cy.xpath("//input[@type='text'][@name='BetweenMin5Max10']").should("be.visible").type('7{enter}').should('have.value','7');
       cy.xpath("//input[@type='text'][@name='Date']").should("be.visible").type('2022-09-30{enter}').should('have.value','2022-09-30');
       cy.xpath("//input[@type='text'][@name='email']").should("be.visible").type('automation.pm4@gmail.com').should('have.value','automation.pm4@gmail.com');
       cy.xpath("//input[@type='text'][@name='In7']").should("be.visible").type('7').should('have.value','7');
       cy.xpath("//input[@type='text'][@name='MaxLength12']").should("be.visible").type('123456789000').should('have.value','123456789000');
       cy.xpath("//input[@type='text'][@name='MinLength3']").should("be.visible").type('123').should('have.value','123');
       cy.xpath("//input[@type='text'][@name='NotIn9']").should("be.visible").type('8').should('have.value','8');
       cy.xpath("//input[@type='text'][@name='form_input_13']").should("be.visible").type('xyz').should('have.value','xyz');
       cy.xpath("//input[@type='text'][@name='form_input_15']").should("be.visible").type('required').should('have.value','required');
       cy.xpath("//input[@type='text'][@name='RequiredIfdateStart']").should("be.visible").type('2021-09-01{enter}').should('have.value','2021-09-01');
       cy.xpath("//input[@type='text'][@name='form_input_12']").should("be.visible").type('2021-09-02{enter}').should('have.value','2021-09-02');
       cy.xpath("//input[@type='text'][@name='form_input_16']").should("be.visible").type('automation.pm4@gmail.com').should('have.value','automation.pm4@gmail.com');
       cy.xpath("//input[@type='text'][@name='form_input_17']").should("be.visible").type('https://qualitlabs-qa.processmaker.net/').should('have.value','https://qualitlabs-qa.processmaker.net/');
       //Date Type
       cy.xpath("//input[@type='text'][@name='TEXT']").should("be.visible").type('TEXT').should('have.value','TEXT');
       cy.xpath("//input[@type='number'][@name='INTEGER']").should("be.visible").type('73042172').should('have.value','73042172');
       cy.xpath("//input[@type='text'][@name='CURRENCY']").should("be.visible").type('100{enter}').should('have.value','100,00');
       cy.xpath("//input[@type='text'][@name='PERCENTAGE']").should("be.visible").type('90{enter}').should('have.value','90.00 %');
       cy.xpath("//input[@type='number'][@name='DECIMAL']").should("be.visible").type('12.3{enter}').should('have.value','12.3');
       cy.xpath("//input[@type='text'][@name='DATETIME']").should("be.visible").type('1993-09-30 12:00{enter}');
       cy.xpath("//input[@type='text'][@name='DATE']").should("be.visible").type('1993-09-30{enter}').should('have.value','1993-09-30');
       cy.xpath("//input[@type='password'][@name='PASSWORD']").should("be.visible").type('Colosa123{enter}').should('have.value','Colosa123');
       //Combinations
       cy.xpath("//input[@type='text'][@name='form_input_11']").should("be.visible").type('125').should('have.value','125.00 BOB');
       cy.xpath("//input[@type='password'][@name='form_input_14']").should("be.visible").type('Colosa');
       cy.xpath("//input[@type='number'][@name='form_input_18']").should("be.visible").type('29').should('have.value','29');
       cy.xpath("//input[@type='text'][@name='form_input_19']").should("be.visible").type('125').should('have.value','125.00 %');
       //submit
       cy.get(':nth-child(4) > .form-group > .btn').should("be.visible").click({ multiple: true });
       ...
       //Review form task
       cy.get('[class="card card-body border-top-0 h-100 form-screen"]').should("be.visible");
       cy.xpath('//text()[contains(.,"New Date Picker")]/ancestor::div[@data-cy="screen-field-form_date_picker_1"]//input').should("be.visible").should('have.value','2022-09-30 19:01');
       cy.xpath("//strong[text()='Nested Validations']").should("be.visible");
       //Options Menu
       cy.xpath("//input[@type='text'][@name='form_input_1']").should("be.visible");
       cy.xpath("//input[@type='text'][@name='form_input_2']").should("be.visible").should('have.value','place holder');
       cy.xpath("//input[@type='text'][@name='form_input_3']").should("be.visible").should('have.value','helper text');
       cy.xpath("//small[text()='helper text']").should("be.visible");
       cy.xpath("//input[@type='text'][@name='design']").should("be.visible").should('have.value','design');
       cy.xpath("//input[@type='text'][@name='form_input_5']").should("be.visible").should('have.value','paos default  value');
       cy.xpath("//input[@type='text'][@name='form_input_4']").should("be.visible").should('have.value','30/09/1993');
       cy.xpath("//input[@type='text'][@name='form_input_7']").should("be.visible").should('have.value','300-05-1993');
       //Validations Rules
       cy.xpath("//span[text()='Validations Rules']").should("be.visible");
       cy.xpath('//text()[contains(.,"dateStart")]/ancestor::div[@data-cy="screen-field-dateStart"]//input').should("be.visible").should('have.value','2022-09-30');
       cy.xpath("//input[@type='text'][@name='form_input_8']").should("be.visible").should('have.value','yes');
       cy.xpath("//input[@type='text'][@name='form_input_9']").should("be.visible").should('have.value','2022-10-30');
       cy.xpath("//input[@type='text'][@name='form_input_10']").should("be.visible").should('have.value','2022-09-30');
       cy.xpath("//input[@type='text'][@name='Alpha']").should("be.visible").should('have.value','Mayita');
       cy.xpath("//input[@type='text'][@name='AlphaNumeric']").should("be.visible").should('have.value','Mayita123');
       cy.xpath("//input[@type='text'][@name='BeforeDate']").should("be.visible").should('have.value','2022-08-30');
       cy.xpath("//input[@type='text'][@name='BeforeorEqualtoDate']").should("be.visible").should('have.value','2022-08-30');
       cy.xpath("//input[@type='text'][@name='BetweenMin5Max10']").should("be.visible").should('have.value','7');
       cy.xpath("//input[@type='text'][@name='Date']").should("be.visible").should('have.value','2022-09-30');
       cy.xpath("//input[@type='text'][@name='email']").should("be.visible").should('have.value','automation.pm4@gmail.com');
       cy.xpath("//input[@type='text'][@name='In7']").should("be.visible").should('have.value','7');
       cy.xpath("//input[@type='text'][@name='MaxLength12']").should("be.visible").should('have.value','123456789000');
       cy.xpath("//input[@type='text'][@name='MinLength3']").should("be.visible").should('have.value','123');
       cy.xpath("//input[@type='text'][@name='NotIn9']").should("be.visible").should('have.value','8');
       cy.xpath("//input[@type='text'][@name='form_input_13']").should("be.visible").should('have.value','xyz');
       cy.xpath("//input[@type='text'][@name='form_input_15']").should("be.visible").should('have.value','required');
       cy.xpath("//input[@type='text'][@name='RequiredIfdateStart']").should("be.visible").should('have.value','2021-09-01');
       cy.xpath("//input[@type='text'][@name='form_input_12']").should("be.visible").should('have.value','2021-09-02');
       cy.xpath("//input[@type='text'][@name='form_input_16']").should("be.visible").should('have.value','automation.pm4@gmail.com');
       cy.xpath("//input[@type='text'][@name='form_input_17']").should("be.visible").should('have.value','https://qualitlabs-qa.processmaker.net/');
       //Date Type
       cy.xpath("//input[@type='text'][@name='TEXT']").should("be.visible").should('have.value','TEXT');
       cy.xpath("//input[@type='number'][@name='INTEGER']").should("be.visible").should('have.value','73042172');
       cy.xpath("//input[@type='text'][@name='CURRENCY']").should("be.visible").should('have.value','100,00');
       cy.xpath("//input[@type='text'][@name='PERCENTAGE']").should("be.visible").should('have.value','90.00 %');
       cy.xpath("//input[@type='number'][@name='DECIMAL']").should("be.visible").should('have.value','12.3');
       cy.xpath("//input[@type='text'][@name='DATETIME']").should("be.visible").should('have.value','1993-09-30 12:00');
       cy.xpath("//input[@type='text'][@name='DATE']").should("be.visible").should('have.value','1993-09-30');
       cy.xpath("//input[@type='password'][@name='PASSWORD']").should("be.visible").should('have.value','Colosa123');
       cy.contains('Combinations').scrollIntoView();
       //Combinations
       cy.xpath("//input[@type='text'][@name='form_input_11']").should("be.visible").should('have.value','125.00 BOB');
       cy.xpath("//input[@type='password'][@name='form_input_14']").should("be.visible");
       cy.xpath("//input[@type='number'][@name='form_input_18']").should("be.visible").should('have.value','29');
       cy.xpath("//input[@type='text'][@name='form_input_19']").should("be.visible").should('have.value','125.00 %');
       //submit
       cy.get(':nth-child(4) > .form-group > .btn').should("be.visible").click({ multiple: true });
       //Review Summary and forms
       cy.xpath("//a[contains(text(),'Summary')]").should("be.visible");
    ...
    async CompleteFormsTCP42274(requestId){
        cy.get('[class="card card-body border-top-0 h-100 form-screen"]').should("be.visible");
        cy.xpath("//input[@type='text'][@class='form-control']").should("be.visible").type('2022-09-30 18:24{enter}').should('have.value','2022-09-30 18:24');
        //Sign
        cy.get('[data-cy="screen-field-sig1"] > .signature-container > .signature > canvas').should("be.visible").click('center');
        cy.get('[data-cy="screen-field-sig1"] > .signature-container > .signature > canvas').should("be.visible").click('left');
        cy.get('[data-cy="screen-field-sig1"] > .signature-container > .signature > canvas').should("be.visible").click('right');

        cy.get('[data-cy="screen-field-sigNotrequired"] > .signature-container > .signature > canvas').should("be.visible").click('center');
        cy.get('[data-cy="screen-field-sigNotrequired"] > .signature-container > .signature > canvas').should("be.visible").click('left');
        cy.get('[data-cy="screen-field-sigNotrequired"] > .signature-container > .signature > canvas').should("be.visible").click('right');
        //Sign loop New Array of Objects
        cy.get('[data-cy="screen-field-sig2"] > .signature-container > .signature > canvas').should("be.visible").click('center');
        cy.get('[data-cy="screen-field-sig2"] > .signature-container > .signature > canvas').should("be.visible").click('left');
        cy.get('[data-cy="screen-field-sig2"] > .signature-container > .signature > canvas').should("be.visible").click('right');
        //add loop
        cy.get('[data-cy="loop-loop_1-add"]').should("be.visible").click();
        cy.get(':nth-child(2) > .container-fluid > :nth-child(1) > .page > :nth-child(1) > [data-cy="screen-field-sig2"] > .signature-container > .signature > canvas').should("be.visible").click('center');
        cy.get(':nth-child(2) > .container-fluid > :nth-child(1) > .page > :nth-child(1) > [data-cy="screen-field-sig2"] > .signature-container > .signature > canvas').should("be.visible").click('left');
        cy.get(':nth-child(2) > .container-fluid > :nth-child(1) > .page > :nth-child(1) > [data-cy="screen-field-sig2"] > .signature-container > .signature > canvas').should("be.visible").click('right');
        //Sign loop - Existing Array
        cy.get('[data-cy="loop-loop_2-add"]').should("be.visible").click();
        cy.get('[data-cy="screen-field-sig3"] > .signature-container > .signature > canvas').should("be.visible").click('center');
        cy.get('[data-cy="screen-field-sig3"] > .signature-container > .signature > canvas').should("be.visible").click('left');
        cy.get('[data-cy="screen-field-sig3"] > .signature-container > .signature > canvas').should("be.visible").click('right');
        //add loop
        cy.get('[data-cy="loop-loop_2-add"]').should("be.visible").click();
        ...
        cy.get(':nth-child(2) > .container-fluid > :nth-child(1) > .page > :nth-child(1) > [data-cy="screen-field-sig3"] > .signature-container > .signature > canvas').should("be.visible").click('left');
        cy.get(':nth-child(2) > .container-fluid > :nth-child(1) > .page > :nth-child(1) > [data-cy="screen-field-sig3"] > .signature-container > .signature > canvas').should("be.visible").click('right');
        //Sign Record list
        cy.get('[data-cy="add-row"]').should("be.visible").click();
        cy.get('[class="modal-content"]').first().should("be.visible");
        //Sing
        cy.xpath('//h5[text()="Add"]//ancestor::div[@class="modal-content"]//div[@data-cy="screen-field-ss1"]//div[@class="signature pl-0"]').should("be.visible").click('center');
        cy.xpath('//h5[text()="Add"]//ancestor::div[@class="modal-content"]//div[@data-cy="screen-field-ss1"]//div[@class="signature pl-0"]').should("be.visible").click('left');
        cy.xpath('//h5[text()="Add"]//ancestor::div[@class="modal-content"]//div[@data-cy="screen-field-ss1"]//div[@class="signature pl-0"]').should("be.visible").click('right');
        //Loop RL sing
        cy.xpath('//h5[text()="Add"]//ancestor::div[@class="modal-content"]//div[@name="S-RL"]/div[2]//div[@data-cy="screen-field-ss2"]/div/div[2][@class="signature pl-0"]').should("be.visible").click('center');
        cy.xpath('//h5[text()="Add"]//ancestor::div[@class="modal-content"]//div[@name="S-RL"]/div[2]//div[@data-cy="screen-field-ss2"]/div/div[2][@class="signature pl-0"]').should("be.visible").click('left');
        cy.xpath('//h5[text()="Add"]//ancestor::div[@class="modal-content"]//div[@name="S-RL"]/div[2]//div[@data-cy="screen-field-ss2"]/div/div[2][@class="signature pl-0"]').should("be.visible").click('right');
        //add loop
        ...
        cy.xpath('//h5[text()="Add"]//ancestor::div[@class="modal-content"]//div[@name="S-RL"]//div/div[2]//div[@data-cy="screen-field-ss2"]/div/div[2][@class="signature pl-0"]').should("be.visible").click('left');
        cy.xpath('//h5[text()="Add"]//ancestor::div[@class="modal-content"]//div[@name="S-RL"]//div/div[2]//div[@data-cy="screen-field-ss2"]/div/div[2][@class="signature pl-0"]').should("be.visible").click('right');
        //Sign loop RL - Existing Array
        cy.get('[data-cy="loop-loop_4-add"]').first().should("be.visible").click();
        ...
        cy.xpath('//h5[text()="Add"]//ancestor::div[@class="modal-content"]//div[@name="S-RL"]/div[2]/div/div[2]//div[@name="loop_4"]//div[@data-cy="screen-field-ss3"]//div[@class="signature pl-0"]').should("be.visible").click('left');
        cy.xpath('//h5[text()="Add"]//ancestor::div[@class="modal-content"]//div[@name="S-RL"]/div[2]/div/div[2]//div[@name="loop_4"]//div[@data-cy="screen-field-ss3"]//div[@class="signature pl-0"]').should("be.visible").click('right');
        cy.xpath(" //button[text()='Ok']").should("be.visible").click();
        //Submit
   ...
   actionsAndAssertionsOfTCP42225(){
    ...
    verifySelectOptionInTCP42214(option){
        var locator1 ='[name="'+option+'"]'
        cy.get(locator1).should('be.checked')
    }
    //TCP4-2202
    actionsAndAssertionsOfTCP42202(){
    ...
    //TCP4 2195
    actionsAndAssertionsOfTCP42195FillRecordList(input1,textArea1,datePicker1,datePicker2){
    ...
    actionsAndAssertionsOfTCP42195FillLoop(index,input2,textArea2,datePicker3,datePicker4){
    ...
    actionsAndAssertionsOfTCP42195RecoverRecordList(index,option,label){
    ...
    actionsAndAssertionsOfTCP42195RecoverRecordListDateTime(index,date,label){
        ...
        cy.xpath(multiselect).eq(index).click();
        cy.xpath(locatorInputOption).eq(index).click({force:true});
        var option = date+'T14:00:00.000Z';
        cy.log(option);
        cy.xpath(locatorInputOption).eq(index).type(option).should('have.value',option);
        cy.xpath(locatorOption)
            .should('have.attr', 'aria-label')
    ...
    actionsAndAssertionsOfTCP42132(requestId){
        ...
        //FILE 4
        cy.xpath('//div[@title="shibboleth.crt"]').click().should("be.visible");
        //cy.xpath('//div[@title="cloud1.jpeg"]').should("be.visible");
        //Save and submit
    ...
    actionsAndAssertionsOfTCP42192(requestId){
        cy.get('[data-cy="screen-field-form_input_1"]').first().should('be.visible').type('test TCP4-2192 automation').should('have.value','test TCP4-2192 automation');
        cy.xpath('(//input[@name="form_input_1"])[2]').should('be.visible').should('have.value','test TCP4-2192 automation');
        cy.xpath('//label[@class="form-check-label"]').should('be.visible').click();
        cy.xpath('//textarea[@name="form_text_area_1"]').should('be.visible').type('test TCP4-2192 Process PDF Generated Nested screen').should('have.value','test TCP4-2192 Process PDF Generated Nested screen');
        cy.xpath('//label[text()="New Select List"]/parent::div//div[@class="multiselect__tags"]').should('be.visible').click();
        cy.xpath('//label[text()="New Select List"]/parent::div//div[@class="multiselect__tags"]//input').should('be.visible').type('Three{enter}');
    ...
    actionsAndAssertionsOfTCP42196(requestId){
        ...
        cy.xpath('//button[@title="Details"]').first().should("be.visible").click();
        cy.xpath('//div[@class="card-body h-100"]').should("be.visible");
        cy.contains('has completed the task Form Task').scrollIntoView();
    }
    actionsAndAssertionsOfTCP42169(requestId){
        cy.get('[data-cy="screen-field-name"]').should('be.visible').type('participant test').should('have.value','participant test');
        ...
        //sing
        cy.xpath('//div[@class="signature pl-0"]').should("be.visible").click('center');
        cy.xpath('//div[@class="signature pl-0"]').should("be.visible").click('left');
        cy.xpath('//div[@class="signature pl-0"]').should("be.visible").click('right');
        //submit
        cy.xpath("//button[contains(text(),'New Submit')]").should('be.visible').click();
        cy.wait(2000);
        //Open task Registration1
        var taskName = 'Registration1';
        cy.visit('/requests/'+requestId);
        request.waitUntilElementIsVisible('selector','[id="requestTabContent"] > [id="pending"] >* a[href^="/tasks"]');
        request.openTaskByTaskName(taskName);
        cy.get('[data-cy="screen-field-name1"]').should('be.visible').type('participant test 1').should('have.value','participant test 1');
        ...
        cy.xpath('(//div[@class="signature pl-0"])[2]').should("be.visible").click('left');
        cy.xpath('(//div[@class="signature pl-0"])[2]').should("be.visible").click('right');
        //submit
        cy.xpath("//button[contains(text(),'New Submit')]").should('be.visible').click();
        cy.wait(2000);
        //Open task Registration2
        var taskName = 'Registration2';
        cy.visit('/requests/'+requestId);
        cy.reload();
        request.waitUntilElementIsVisible('selector','[id="requestTabContent"] > [id="pending"] >* a[href^="/tasks"]');
        request.openTaskByTaskName(taskName);
        cy.get('[data-cy="screen-field-name2"]').should('be.visible').type('participant test 2').should('have.value','participant test 2');
        ...
        cy.xpath('(//div[@class="signature pl-0"])[3]').should("be.visible").click('left');
        cy.xpath('(//div[@class="signature pl-0"])[3]').should("be.visible").click('right');
        //submit
        cy.xpath("//button[contains(text(),'New Submit')]").should('be.visible').click();
        cy.wait(2000);
        //Open task Registration3
        var taskName = 'Registration3';
        cy.visit('/requests/'+requestId);
        request.waitUntilElementIsVisible('selector','[id="requestTabContent"] > [id="pending"] >* a[href^="/tasks"]');
        request.openTaskByTaskName(taskName);
        cy.get('[data-cy="screen-field-name3"]').should('be.visible').type('participant test 3').should('have.value','participant test 3');
        cy.xpath('(//label[text()="SEX"]/parent::div//div[@class="multiselect__tags"])[4]').should('be.visible').click();
        cy.xpath('(//div[@class="multiselect__tags"])[4]').should('be.visible').click();
        //cy.xpath("(//label[text()='SEX']/parent::div//input)[4]").should('be.visible').type('Man{enter}');
        cy.xpath('//ul[@id="listbox-3"]//li[@id="option-3-1"]').should('be.visible').click({force: true});
        //sing
        cy.xpath('(//div[@class="signature pl-0"])[4]').should("be.visible").click('center');
        cy.xpath('(//div[@class="signature pl-0"])[4]').should("be.visible").click('left');
        cy.xpath('(//div[@class="signature pl-0"])[4]').should("be.visible").click('right');
        //submit
        cy.xpath("//button[contains(text(),'New Submit')]").should('be.visible').click();
        cy.wait(2000);
        //Open task Registration4
        var taskName = 'Registration4';
        cy.visit('/requests/'+requestId);
        request.waitUntilElementIsVisible('selector','[id="requestTabContent"] > [id="pending"] >* a[href^="/tasks"]');
        request.openTaskByTaskName(taskName);
        cy.get('[data-cy="screen-field-name4"]').should('be.visible').type('participant test 4').should('have.value','participant test 4');
        ...
        //cy.xpath("(//label[text()='SEX']/parent::div//input)[5]").should('be.visible').type('Man{enter}');
        ...
        cy.xpath('(//div[@class="signature pl-0"])[5]').should("be.visible").click('left');
        cy.xpath('(//div[@class="signature pl-0"])[5]').should("be.visible").click('right');
        cy.wait(2000);
        //Review Total Man and Woman
        ...
        cy.visit('/requests/'+ requestId);
        cy.xpath("//a[contains(text(),'Summary')]").should("be.visible");
        cy.xpath("//td[text()='sex']").should("be.visible");
        cy.xpath("//td[text()='name']").should("be.visible");
        cy.xpath("//td[text()='sex1']").should("be.visible");
        cy.xpath("//td[text()='sex2']").should("be.visible");
        cy.xpath("//td[text()='sex3']").should("be.visible");
        cy.xpath("//td[text()='sex4']").should("be.visible");
        cy.xpath("//td[text()='sign']").should("be.visible");
        cy.xpath("//td[text()='name1']").should("be.visible");
        cy.xpath("//td[text()='name2']").should("be.visible");
        cy.xpath("//td[text()='name3']").should("be.visible");
        cy.xpath("//td[text()='name4']").should("be.visible");
        cy.xpath("//td[text()='sign1']").should("be.visible");
        cy.xpath("//td[text()='sign2']").should("be.visible");
        cy.xpath("//td[text()='sign3']").should("be.visible");
        cy.xpath("//td[text()='sign4']").should("be.visible");
        cy.xpath("//td[text()='totalman']").should("be.visible");
        cy.xpath("//td[text()='totalwoman']").should("be.visible");
        //Review Forms tab
        cy.xpath("//a[contains(text(),'Forms')]").click();
        cy.xpath('//button[@title="Details"]').first().should("be.visible").click();
        cy.xpath('//div[@class="card-body h-100"]').should("be.visible");
        cy.get('[item-index="4"] > .vuetable-slot > .actions > .popout > [title="Details"] > .fas').should("be.visible").click();
        cy.get('#overlay-background > .card').should("be.visible");
        cy.contains('Admin User has completed the task Registration').scrollIntoView();
    }
    actionsAndAssertionsOfTCP42289(processName){
        ...
        login.login();
        //open request
        var processName = "TCP4-2289 Check visibility rules from the web entry";
        request.openRequestByName(processName);
        //open the task
        cy.get('tr > :nth-child(1) > a').first().click();
        //Review data
        ...
        cy.xpath('//button[@title="Details"]').should("be.visible").click();
        cy.xpath('//div[@class="card-body h-100"]').should("be.visible");
        cy.contains('Anonymous User has completed the task reviewData').scrollIntoView();
    }
    //TCP4-2170
    ...
    actionsAndAssertionsAllSelectList(){
        ...
        cy.xpath('//label[text()="Rule=Required If2"]/parent::div//div').should('contain','Field is required');
        cy.get('textarea[name="form_text_area_1"]').type('abc')
        cy.xpath('//label[text()="Rule=Required If2"]/parent::div//div').should('not.exist');
        //TextArea2 Rule=Required2 Required
        cy.xpath('//label[text()="Rule=Required2"]/parent::div//div').should('contain','Field is required');
        cy.get('textarea[name="form_text_area_2"]').type('abc')
        cy.xpath('//label[text()="Rule=Required2"]/parent::div//div').should('not.exist');
    }
    //TCP4-2178
    ...
    actionsAndAssertionsOfTCP42123(requestId){
        ...
        cy.get('[data-cy="screen-field-num"]').should('be.visible').type('123').should('have.value','123');
        //add new first recordlist
        cy.xpath('//button[@data-cy="add-row"]').first().click();
        cy.xpath('//div[@class="modal-content"]').first().should('be.visible');
        //complete data recordlist
        ...
        cy.xpath('//div[@class="data-table overflow-auto"]//table//tbody//tr ').first().should('be.visible');
        cy.xpath('(//div[@class="data-table overflow-auto"]//table//tbody//tr)[2]').should('be.visible');
        cy.xpath('(//div[@class="data-table overflow-auto"]//table//tbody//tr)[3]').should('be.visible');
        cy.get(".card-footer > .btn").should('be.visible').click();
        request.verifyTaskIsCompleted();

        //Go to review request form task 1
        login.navigateToUrl();
        cy.visit('/requests/'+ requestId);
        taskName = 'Form Task 1';
        //request.openTaskByTaskName(taskName);
        var taskXpath = '(//div[@id="requestTabContent"]//div[@id="pending"]//a[contains(text(),"Form Task 1")])[1]';
        cy.xpath(taskXpath).should('be.visible');
        cy.xpath(taskXpath).click();
        cy.xpath("//label[contains(text(),'Nueva entrada')]").should('be.visible');
        cy.get('[data-cy="screen-field-form_input_1"]').should('be.visible').type('multi instance tak parallel tarea 1').should('have.value','multi instance tak parallel tarea 1');
        //submit
        cy.xpath("//button[contains(text(),'New Submit')]").should('be.visible').click();
        request.verifyTaskIsCompleted();
        ...
        cy.xpath(taskXpath).click();
        cy.xpath("//label[contains(text(),'Nueva entrada')]").should('be.visible');
        cy.get('[data-cy="screen-field-form_input_1"]').should('be.visible').type('multi instance tak parallel tarea 2').should('have.value','multi instance tak parallel tarea 2');
        //submit
        cy.xpath("//button[contains(text(),'New Submit')]").should('be.visible').click();
        request.verifyTaskIsCompleted();
        ...
        //Go to review request form task 2
        cy.visit('/requests/'+ requestId);
        taskName = 'Form Task 2';
        //request.openTaskByTaskName(taskName);
        ...
        cy.xpath("//a[contains(text(),'Manual Task')]").should('be.visible');
        cy.xpath("//a[contains(text(),'Forms')]").should('be.visible').click();
        cy.xpath('//button[@title="Details"]').first().should("be.visible").click();
        cy.xpath('//div[@class="card-body h-100"]').should("be.visible");
        cy.contains('has completed the task Form Task').scrollIntoView();
    ...
    actionsAndAssertionsOfTCP42194(processName){
        cy.xpath("//input[@type='text'][@name='name']").should('be.visible').type('test case 2194').should('have.value','test case 2194');
        cy.xpath("//label[contains(text(),'Producto 1')]").should('be.visible').click();
        cy.xpath("//input[@type='text'][@name='content']").should('be.visible');
        cy.xpath("//input[@type='number'][@name='cant']").should('be.visible').type('12');

        cy.xpath("//label[contains(text(),'Producto 2')]").should('be.visible').click();
        cy.xpath("(//input[@type='text'][@name='content'])[2]").should('be.visible');
        cy.xpath("(//input[@type='number'][@name='cant'])[2]").should('be.visible').type('13');

        cy.xpath("//label[contains(text(),'Producto 3')]").should('be.visible').click();
        cy.xpath("(//input[@type='text'][@name='content'])[3]").should('be.visible');
        cy.xpath("(//input[@type='number'][@name='cant'])[3]").should('be.visible').type('14');

        cy.xpath("//label[contains(text(),'Producto 4')]").should('be.visible').click();
        cy.xpath("(//input[@type='text'][@name='content'])[4]").should('be.visible');
        cy.xpath("(//input[@type='number'][@name='cant'])[4]").should('be.visible').type('15');
        //submit
        cy.xpath("//button[contains(text(),'New Submit')]").should('be.visible').click();
        //login
        login.navigateToUrl();
        ...
        request.openRequestByName(processName);
        //Open task
        cy.xpath('//div[@id="requestTabContent"]//div[@id="pending"]//a[contains(text(),"Manual Task")]').should('be.visible').click();
        //Review manual task
        cy.get('[class="card card-body border-top-0 h-100 display-screen"]').should('be.visible');
        cy.xpath('//div[@name="For process1 display"]//div[@class="form-group"]//p').first().should('be.visible');
        cy.xpath('(//div[@name="For process1 display"]//div[@class="form-group"]//p)[2]').should('be.visible');
        cy.xpath('(//div[@name="For process1 display"]//div[@class="form-group"]//p)[4]').should('be.visible');
        cy.xpath('(//div[@name="For process1 display"]//div[@class="form-group"]//p)[5]').should('be.visible');
        cy.xpath('(//div[@name="For process1 display"]//div[@class="form-group"]//p)[6]').should('be.visible');
        cy.xpath('(//div[@name="For process1 display"]//div[@class="form-group"]//p)[7]').should('be.visible');
        cy.xpath('(//div[@name="For process1 display"]//div[@class="form-group"]//p)[8]').should('be.visible');
        cy.xpath('(//div[@name="For process1 display"]//div[@class="form-group"]//p)[9]').should('be.visible');
        cy.xpath('(//div[@name="For process1 display"]//div[@class="form-group"]//p)[10]').should('be.visible');
        cy.xpath('(//div[@name="For process1 display"]//div[@class="form-group"]//p)[11]').should('be.visible');
        //complete manualtask
        cy.xpath("//button[contains(text(),'Complete Task')]").should('be.visible').click();
        request.verifyTaskIsCompleted();
        //open request

        request.gotocompletedrequest(processName);
        //Review Completed task
        cy.xpath("//a[contains(text(),'Completed')]").should("be.visible").click();
        cy.contains('Anonymous User has completed the task PDF Generator').should('be.visible');
        cy.contains('Anonymous User has completed the task Send Email').should('be.visible');
        cy.contains('Anonymous User has completed the task Manual Task').should('be.visible');
        cy.contains('An Anonymous User started this request from a web entry').should('be.visible');
        //Review File Manager
        cy.xpath("//a[contains(text(),'File Manager')]").should("be.visible").click();
        cy.xpath('//button[@title="View"]').should("be.visible").click();
        cy.wait(5000);
    ...
    actionsAndAssertionsOfTCP42474(requestId){
        ...
        cy.xpath("//label[text()='Date Pickerthis is a mustache test in label']//ancestor::div[@data-cy='screen-field-form_date_picker_1']//input").type('2022-11-08');

        cy.xpath("//label[text()='Date Picker Time this is a mustache test in label']").should('be.visible');
        cy.xpath("//label[text()='Date Picker Time this is a mustache test in label']//ancestor::div[@data-cy='screen-field-form_date_picker_2']//input").type('2022-11-01 20:12');
        ...
        cy.xpath("//label[text()='Date Pickerthis is a mustache test in label']").should('be.visible');
        cy.xpath("//label[text()='Date Pickerthis is a mustache test in label']//ancestor::div[@data-cy='screen-field-form_date_picker_1']//input").should('have.value','2022-11-08');

        cy.xpath("//label[text()='Date Picker Time this is a mustache test in label']").should('be.visible');
        cy.xpath("//label[text()='Date Picker Time this is a mustache test in label']//ancestor::div[@data-cy='screen-field-form_date_picker_2']//input").should('have.value','2022-11-01 20:12');
        ...
        cy.xpath('//button[@title="View"]').should("be.visible").click();
        cy.wait(2000);
        cy.get('.text-center > div > .mw-100').should("be.visible");
        //Review Forms tab
        cy.xpath("//a[contains(text(),'Forms')]").click();
        cy.xpath('//button[@title="Details"]').first().should("be.visible").click();
        cy.xpath('//div[@class="card-body h-100"]').should("be.visible");
    ...
    actionsAndAssertionsOfTCP42193(requestId){
        cy.xpath("//label[text()='New Date Picker']/parent::div//input").should("be.visible").type('2022-11-11 14:21{enter}').should('have.value', '2022-11-11 14:21');
        cy.xpath("//label[text()='Var1']/parent::div//input").should("be.visible").type('1{enter}').should('have.value','1');
        cy.xpath("//label[text()='New Input']/parent::div//input").first().should("be.visible").type('test case').should('have.value', 'test case');
        cy.xpath("(//label[text()='New Checkbox']/parent::div//input)[1]").should("be.visible").click();
        cy.xpath("(//label[text()='New Input']/parent::div//input)[2]").should("be.visible").type('test case 2').should('have.value', 'test case 2');
        cy.xpath("(//label[text()='New Checkbox']/parent::div//input)[2]").should("be.visible").click();
        //add loop
        cy.get('[data-cy="loop-loop_1-add"]').should("be.visible").click();
        cy.xpath("(//label[text()='New Input']/parent::div//input)[3]").should("be.visible").type('add test case').should('have.value', 'add test case');
        cy.xpath("(//label[text()='New Checkbox']/parent::div//input)[3]").should("be.visible").click();
        //submit
        cy.xpath("//button[contains(text(),'New Submit')]").should("be.visible").click();
        cy.wait(5000);
        //Complete task form 2
        var taskName = 'Manual Task';
        cy.visit('/requests/'+requestId);
        //Wait by 1 minute until the boundary timer is executed
        cy.wait(60000)
        cy.wait(60000)
        cy.reload();
        request.waitUntilElementIsVisibleCant('[id="requestTabContent"] > [id="pending"] >* a[href^="/tasks"]');
        request.openTaskByTaskName(taskName);
        //review manual task
        cy.xpath('//div[@class="card card-body border-top-0 h-100 display-screen"]').should("be.visible");
        cy.xpath('//div[@class="card card-body border-top-0 h-100 display-screen"]//div[@class="col-sm-6"][2]').should("be.visible");
        cy.xpath('//div[@class="card card-body border-top-0 h-100 display-screen"]//div[@class="page"]/div[2]//div[@class="form-group"]').should("be.visible");
        cy.xpath('//div[@class="card card-body border-top-0 h-100 display-screen"]//div[@class="page"]/div[3]//div[@class="form-group"]').should("be.visible");
        cy.xpath('//div[@class="card card-body border-top-0 h-100 display-screen"]//div[@class="page"]/div[5]//div[@class="form-group"]').should("be.visible");
        cy.xpath('//div[@class="card card-body border-top-0 h-100 display-screen"]//div[@class="page"]/div[7]//div[@class="form-group"]').should("be.visible");
        cy.xpath("//button[contains(text(),'Complete Task')]").should("be.visible").click();
        //Review Summary and forms
        cy.xpath("//a[contains(text(),'Summary')]").should("be.visible");
        cy.contains('Admin User has completed the task Form Task').scrollIntoView();
