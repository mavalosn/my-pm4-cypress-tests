...
export class Screens {
	...
	searchForAScreen(screenName) {
		...
		cy.xpath(Selectors.searchInput).type(screenName).should('have.value', screenName);
		cy.wait(1500);
	}
	...
	verifyPresenceOfScreenAndImportScreen(screenName, filePath) {
		...
		cy.get(Selectors.screenTabelBx).then((flag) => {
			cy.log(flag);
			if (flag.find(Selectors.screenVerifyBx).length > 0) {
				cy.xpath(Selectors.textInputValue.replace("screenName", screenName)).should('be.visible');
			}
			else {
	...
	importScreen(filePath) {
		cy.xpath(Selectors.importPlusBtn).click();
		cy.window().then((win) => {
	 ...
	 **/
	selectDay(day) {
		cy.get(Selectors.pressDayBtn).should('be.visible');
		cy.contains(Selectors.pressDayBtn, day).click();
	}
	 ...
	 **/
	selectHour(hour) {
		cy.get(Selectors.pressCustomHour).should('be.visible');
		cy.get(Selectors.pressCustomHour).click();
		cy.contains(Selectors.selectCustomHour, hour).click();
	}
	 ...
	 **/
	selectMinute(minute) {
		cy.get(Selectors.pressCustomMinute).should('be.visible');
		cy.get(Selectors.pressCustomMinute).click();
		cy.contains(Selectors.selectCustomMinute, minute).click();
	}
	 ...
	 **/
	selectMonth() {
		cy.get(Selectors.pressCenterOption).should('be.visible');
		cy.get(Selectors.pressCenterOption).eq(0).click();
	}
	 ...
	 **/
	selectYear() {
		cy.get(Selectors.pressCenterOption).eq(1).should('be.visible');
		cy.get(Selectors.pressCenterOption).eq(1).click();
	}
	...
	useCustomDate(year, month, day) {
		this.selectMonth();
		this.selectYear();
		this.selectCustomYear(year);
		this.selectCustomMonth(month);
		this.selectDay(day);
	}
